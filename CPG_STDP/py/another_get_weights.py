...
class NC_holder:
    ...
    def add_excitatory_netcons(self, obj):
        self.excitatory_netcons.append(obj)

    def add_inhibitory_netcons(self, obj):
        self.inhibitory_netcons.append(obj)

    def record_excitatory_netcons(self):
        weight_vec = []
        for netcon in self.excitatory_netcons_lists[0]:
            vec = h.Vector(np.zeros(int(time_sim / 0.025 + 1), dtype=np.float32))
            vec.record(netcon.weight)  # ??? _ref_weights[0]
    ...
    def record_inhibitory_netcons(self):
        weight_vec = []
        for netcon in self.inhibitory_netcons_lists[0]:
            vec = h.Vector(np.zeros(int(time_sim / 0.025 + 1), dtype=np.float32))
