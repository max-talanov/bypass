...
nInt = 19 #19 # 196 # Number of neurons in interneuronal pools
N = 5 #5 #50  #Cell amount
...
class NC_holder:
    def __init__(self):
        ...
        self.inhibitory_netcons_lists = []
        self.excitatory_weights_vec = h.Vector()
        self.inhibitory_weights_vec = h.Vector()

    def add_excitatory_netcons(self, obj):
        self.excitatory_netcons_lists.append(obj)

    def add_inhibitory_netcons(self, obj):
        self.inhibitory_netcons_lists.append(obj)

    def record_excitatory_netcons(self):
        weight_vec = []
        for netcon in self.excitatory_netcons_lists:
            vec = h.Vector()
            vec.record(netcon._ref_weight[0])  # ??? _ref_weights[0]
            weight_vec.append(vec)
        self.excitatory_weights_vec = weight_vec
        return weight_vec

    def record_inhibitory_netcons(self):
        weight_vec = []
        for netcon in self.inhibitory_netcons_lists:
            vec = h.Vector()
            vec.record(netcon._ref_weight[0])
            weight_vec.append(vec)
        self.inhibitory_weights_vec = weight_vec
        return weight_vec
    ...
    def get_weights(self):
        ...
        for i in range(nhost):
            if i == rank:
                outavg = []
                for j in range(len(self.inhibitory_netcons_lists)):  # len(self.inhibitory_netcons_lists)
        ...
        if rank == 0:
            logging.info("start recording weights")
            result = np.mean(np.array(result), axis = 0, dtype=np.float32)
            with hdf5.File('./res/inh_WEIGHTS_sp_{}_CVs_{}_bs_{}.hdf5'.format(speed, CV_number, bs_fr), 'w') as file:
...
def weights_record(pool, extra = False):
    ''' Records spikes from gids
      Parameters
      ----------
      pool: list
